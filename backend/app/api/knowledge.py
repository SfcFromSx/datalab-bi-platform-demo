...
async def generate_knowledge(
    ...
    await knowledge_graph.delete_for_datasource(session, data.datasource_id)
    await knowledge_graph.populate_from_knowledge(session, knowledge, data.datasource_id)
...
async def search_knowledge(
    ...
    results = await knowledge_retriever.retrieve(
        session, query, datasource_id=datasource_id, top_k=top_k
    )
...
async def get_knowledge_graph(
    ...
    nodes = await knowledge_graph.get_nodes_by_datasource(session, datasource_id)
    tree = await knowledge_graph.get_tree(session)
